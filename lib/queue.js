/*!
 *This is a hacked versio of firebase-queue
...
var _ = require('lodash'),
    ...
    QueueWorker = require('./queue_worker.js');
...
function Queue() {
  ...
  if (constructorArguments.length < 2) {
    ...
    //hack
    self.tasksRefPath = options.tasksRefPath;
    self.specsRefPath = options.tasksRefPath;
  ...
  for (var i = 0; i < self.numWorkers; i++) {
    ...
    self.workers.push(new QueueWorker(
        //hack
      self.ref.child(self.tasksRefPath||'tasks'),
      processId,
      self.sanitize,
      self.suppressStack,
  ...
  if (_.isUndefined(self.specId)) {
    ...
    //hack
    self.specChangeListener = self.ref.child(self.specsRefPath||'specs').child(self.specId).on(
      'value',
...
Queue.prototype.shutdown = function() {
  ...
  if (!_.isNull(self.specChangeListener)) {
    //hack
    self.ref.child(self.specsRefPath||'specs').child(self.specId).off('value',
