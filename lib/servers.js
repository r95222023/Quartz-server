...
function ServerGroup(config) {
    ...
    ///// init monitors
    var serverMonitorRefUrl = config.SERVER_INFO && config.SERVER_INFO.serverMonitorRefUrl ? (config.SERVER_INFO.serverMonitorRefUrl + '/' + this.serverId) : (gerneralConfig.FBURL + gerneralConfig.SERVER_PATH + '/' + this.serverId + '/monitor'),
        monitors = ['cpu'];
    for (var i = 0; i < monitors.length; i++) {
        this.monitor(serverMonitorRefUrl+'/'+monitors[i], sysMonitor.monitorCpuUsage);
    }
}


ServerGroup.prototype.monitor = function (monitorRefUrl,  getDataFunc) {
//TODO: use socket.io;
    var stopWatch;
