...
function ServerGroup(config) {
    ...
    serverRef.child('online').child(config.SERVER_ID).once(function (snap) {
        if(snap.val()===null){
            regServer(config.SERVER_ID);
        } else {
            var tempServerId = 'temp:'+start.getTime();
            console.log('Server '+config.SERVER_ID+' already exists.');
            console.log('start temporary server: '+tempServerId);
            regServer('temp:'+start.getTime(), true);
        }
    });

    function regServer(serverId, isTempt){
        ...
        if(isTempt) serverRef.child('detail').child(serverId).onDisconnect().set(null);
    }
