...
function ServerGroup(config) {
    ...
    keepOnline(that.serverId, isTemp);
    ...
    function regServer(serverId, isTemorary, isReconnect) {
        ...
        //TODO: use socket.io;
        var stopWatchCPU,
            serverMonitorRefUrl = config.SERVER_INFO&&config.SERVER_INFO.serverMonitorRefUrl? (config.SERVER_INFO.serverMonitorRefUrl+'/'+serverId+'/cpu'):(gerneralConfig.FBURL+gerneralConfig.SERVER_PATH+'/'+serverId+'/monitor/cpu');
        var serverMonitorRef = firebaseUtil.ref(serverMonitorRefUrl);
        serverMonitorRef.child('_watch').on('value', function (snap) {
            var usageRef = serverMonitorRef.child('usage'),
                timeouts = {};
            ...
            if(snap.val()!==null) {
                ...
                stopWatchCPU = sysMonitor.monitorCpuUsage(function (cpuUsage) {
                    ...
                    usageRef.child(now);
                    usageRef.child(now).set(cpuUsage);
