...
function ServerGroup(config) {
    ...
    serverRef.child('online').child(config.SERVER_ID).once('value', function (snap) {
        ...
        if (snap.val() === null) {
            ...
            serverId = 'temp:' + start.getTime();
            console.log('Server ' + config.SERVER_ID + ' already exists.');
            keepOnline(serverId, true);
        }
    });
    ...
    function keepOnline(serverId, isTemporary) {
        ...
        ref.on('value', function (snap) {
            if(snap.val()===null) {
                if(isReconnected) console.log('reconnected');
                regServer(serverId, isTemporary);
                isReconnected=true;
            }
        });
    }

    function regServer(serverId, isTemorary) {
        ...
        if (isTemorary) serverRef.child('detail').child(serverId).onDisconnect().set(null);
