...
function SystemMonitor(conf) {
    ...
    this.thisServerRef = this.serversRef.child(os.hostname());
...
SystemMonitor.prototype.overrideConsole = function (console, maxlines) {
    ...
    ["log", "warn", "error"].forEach(function (method) {
        ...
        console[method] = function () {
            ...
            self.consoleRef.child(lines).set([method].concat(args));
...
//// 每隔sampleInterval執行一次onRetrieveData(cpuUsage), return: 一個function 執行後可取消監視
SystemMonitor.prototype.monitorCpuUsage = function (onRetrieveData, sampleInterval) {
    getCpuUsageAverage(onRetrieveData);
...
function getCpuUsageAverage(cb) {
    //for UNIX-like system
    if(os.loadavg()[0]){cb(os.loadavg()[0]*100);return}
