...
PathMonitor.prototype = {
    ...
    _childChanged: function (snap, data) {
        var self = this;
        if (!cache[snap.toString()]) {
            function updateindex(self, snap, data) {
                ...
                self.esc.index({
                    ...
                    if (error) {
                        ...
                        //prevent indexing when update version;
                        cache[snap.toString()] = 'updating version';
            ...
            //debounce to avoid broadcast storm
            cache[snap.toString()] = {
                debounce: debounce(updateindex, self.debounce || 2000),
                update: updateindex
            };
            cache[snap.toString()].debounce(self, snap, data);

        } else if (cache[snap.toString()]==='updating version') {
            delete cache[snap.toString()];
        } else {
            cache[snap.toString()].debounce(self, snap, data)
        }
    },
