...
PathMonitor.prototype = {
    ...
    _childChanged: function (snap, data) {
        ...
        if(!this.singleServer){
            var escape = false,
                version = snap.val()['_index'][self.hostname]['_version'];
            //escape when queue is processing
            if(snap.val()['_owner']) escape=true;
            for(var server in snap.val()['_index']){
                //escape when all server is creating index
                if(snap.val()['_index'][server]['created']) escape=true;
                //escape when other server is update index
                if(server!==self.hostname&&snap.val()['_index'][server]['_version']===version+1) escape=true;
            }
            if(escape) return;
        }
