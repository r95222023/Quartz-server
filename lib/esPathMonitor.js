...
PathMonitor.prototype = {
    _init: function () {
        ...
        this.removeLeftMonitor = this.ref
            .orderByChild('_remove')
            .equalTo(true)
    ...
    _emit: function () {
        var emitter = this.esc.emitter;
        if (emitter&&emitter.emit){
            emitter.emit(arguments[0],arguments[1]);
            if(arguments[0]!=='error') emitter.emit('index_changed',arguments[1]);
        }
    },
        ...
        self.esc.index({
            ...
            if (error) {
                ...
                snap.ref().child('_index/' + self.hostname).set(response, function (fberr) {
                    ...
                    self._emit('index_added', {type: snap.child('_index/' + self.hostname+'/_type').val()}||'')
                });
            }
        });
    },
    ...
    _childChanged: function (snap, data, sync) {
        var self = this;
        if (_.isEqual(cache[snap.toString()], data)||data._remove) return;

        if (!updateDebounce[snap.toString()]) {
            updateDebounce[snap.toString()] = util.debounce(update, 500);
        }
        updateDebounce[snap.toString()](snap, data, sync);

        function update(snap, data, sync) {
            cache[snap.toString()] = data;
            delete updateDebounce[snap.toString()];
            ...
            function onIndex(error, response) {
                self.failsafe();
                if (error) {
                    ...
                    serverSnap.ref().update({'_version': currentVersion, '_is_outdated': null}, updateMeta);

                    function updateMeta() {
                        console.log('updated', name);

                        self._emit('index_updated', {type: snap.child('_index/' + self.hostname+'/_type').val()}||null);
                        ...
                        if (!sync) {
                            if (versionCheckDebounce[snap.toString()]) {
                                ...
                                function syncVersion() {
                                    snap.child('_index').ref().once('value', function (latestIndexSnap) {
                                        ...
                                        if (currentVersion < latestVersion) setTimeout(function () {
                                            latestIndexSnap.child(self.hostname).ref().update({
                                ...
                                versionCheckDebounce[snap.toString()] = util.debounce(syncVersion, 6000);
                                versionCheckDebounce[snap.toString()]();
                            }
                        } else {
    ...
    _childRemoved: function (snap, data) {
        ...
        this.esc.delete({
            ...
            if (error) {
                ...
                self._emit('index_removed', {type: snap.child('_index/' + self.hostname+'/_type').val()}||'');
            }
        });
    }
};
