        ...
        self.esc.index({
            ...
            if (error) {
                ...
                versionSnap.ref().set(newVersion, function () {
                    ...
                    if (!sync) {
                        if (debounces[snap.toString()]) {
                            debounces[snap.toString()]();
                        } else {

                            debounces[snap.toString()] = debounce(
                                function () {
                                    snap.child('_index').ref().once('value', function (latestIndexSnap) {
                                        var latestVersion = getLatestVersion(latestIndexSnap);

                                        if(newVersion<latestVersion) latestIndexSnap.child(self.hostname + '/_version').ref().set(latestVersion);
                                    ...
                                    delete debounces[snap.toString()];
                                }
                                , 10000);
                            debounces[snap.toString()]();
                        }
