...
PathMonitor.prototype = {
    ...
    _childChanged: function (snap, data, sync) {
        getIndexBeforChanged(snap.toString(), snap.val()['_index']);
        console.log(sync);
        if (_.isEqual(cache[snap.toString()], data)&&!sync) return;
        cache[snap.toString()] = data;
        var self = this;
                ...
                versionSnap.ref().set(parseInt(versionSnap.val()) + 1, function () {
                    ...
                    setTimeout(function () {
                        snap.child('_index').ref().once('value', function (latestIndexSnap) {
                            console.log(JSON.stringify(index[snap.toString()]));
                            console.log(JSON.stringify(latestIndexSnap.val()));
                            for(var key in latestIndexSnap.val()){
                                //the server is offline if the version is not updated

                                if(index[snap.toString()][key]&&latestIndexSnap.val()[key]['_version']===index[snap.toString()][key]['_version']){
